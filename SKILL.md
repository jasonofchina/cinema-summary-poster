...
Default (if no special request):
默认方案（用户无特殊要求时）：
- Title/Subtitle: Songti Regular / 标题 / 小标题：宋体 常规
- Body: Kaiti Regular / 正文内容：楷体 常规

When user specifies special fonts (calligraphy, art fonts, etc.):
用户指定特殊字体时（花体、书法体、艺术字等）：
- Use AI to generate font effects, ensuring text is clear and readable / 使用 AI 生成字体效果，确保文字内容清晰可读
- Strictly no garbled text, missing characters, or merged strokes / 严禁出现乱码、缺字、笔画粘连
- If readability drops, confirm with user to downgrade to default fonts / 若特殊字体导致可读性下降，需向用户确认是否降级为默认字体

General Typography Requirements:
通用排版要求：
...
风格匹配参考：
- Epic/Period: Film grain, Oil painting | Parchment, Xuan paper / 史诗/古装：胶片质感、油画质感 | 羊皮纸、宣纸
- Sci-fi/Cyberpunk: 3D render, Digital style | Plain electronic background / 科幻/赛博：3D渲染、数字风格 | 普通电子背景色
- Indie/Romance: Watercolor, Film | Watercolor paper, Sketch paper / 文艺/爱情：水彩、胶片 | 水彩纸、素描纸
- Animation/Comedy: Illustration, Comic | Coated paper, Print paper / 动画/喜剧：插画、漫画风格 | 铜版纸、打印纸
- Suspense/Thriller: High contrast, Dark tone | Kraft paper, Vintage paper / 悬疑/惊悚：高对比、暗调 | 牛皮纸、复古怀旧纸
- Documentary: Realistic, Photojournalism | Sketch paper, Print paper / 纪录片：写实、纪实摄影风 | 素描纸、打印纸

️ For Sci-fi backgrounds, avoid digital streams, code rain, etc. Keep it clean.
️ 科幻背景时不要出现数字流、代码雨等大科幻元素，保持简洁。
...
- User requests real person portrait: Explain limitations, suggest stylized/illustrated treatment / 用户要求添加现实人物
